...
coverage_plot <- function(data, meta, id=TRUE, color_package=TRUE) {
  ...
  if ("ProteinNames" %in% colnames(data)) {
    ...
    p <- ggplot(data_annotated, aes(x = Sample, y = Value, fill = condition)) +
      stat_summary(fun = sum, geom = "bar", position = "dodge") +
      theme_minimal() +
      labs(title = "Phosphosites per sample", x = "Condition", y = "Number of phosphosites", fill = "Condition") +
...
missing_value_plot <- function(data, meta, bin = 0) {
  ...
  if (bin > 0) {
    ...
    levels_vec <- as.character(0:max(na_count))
  }
...
pca_plot <- function(data, meta, color_package=TRUE) {
  ...
  tryCatch({
    ...
    if (length(zero_variance_columns) > 0) {
      removed_columns <- colnames(transposed_expr)[zero_variance_columns]
      #print(paste("Removed columns with zero variance:", paste(zero_variance_columns, collapse = ", ")))
...
missing_value_plot_prec <- function(data2, meta) {
  ...
  p <- ggplot(data = miss_vals, aes(x = na_count, y = Freq)) +
    geom_bar(stat = "identity", fill = "blue") + 
    geom_text(aes(label = Freq), vjust = -0.5) +  
    labs(x = "Number of Missing Values", y = "Frequency", title = "Missing value plot - precursor level") + 
    theme_minimal() +
...
missing_value_plot_pep <- function(data2, meta, bin=0) {
  if ("File.Name" %in% names(df)){
    ...
    p <- ggplot(data = miss_vals, aes(x = na_count, y = Freq)) +
      geom_bar(stat = "identity", fill = "blue") + 
      geom_text(aes(label = Freq), vjust = -0.5) +  
      labs(x = "Number of Missing Values", y = "Frequency", title = "Missing value plot - peptide level") + 
      theme_minimal() +
      theme(panel.grid.major.x = element_blank(),
            panel.grid.minor.x = element_blank(),
            panel.grid.minor.y = element_blank())
    ...
    na_count <- rowSums(is.na(data2_filtered))
    ...
    plot_title <- "Missing value plot - peptide level"
...
coverage_plot_pep <- function(data2, meta, id = TRUE) {
  if ("File.Name" %in% names(df)){
    ...
    coverage_plot <- ggplot(sum_df, aes(x = Sample, y = Sum, fill = condition)) +
      ...
      labs(title = "Peptides per sample", x = "Samples", y = "Sum of Precursor Quantities", fill = "Condition") +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
      geom_hline(yintercept = nrow(data_filtered), linetype = "dashed", color = "red") +
      xlab("Condition") + ylab("Number of peptides")
    ...
    p <- ggplot(data2_annotated, aes(x = Sample, y = Value, fill = condition)) +
      stat_summary(fun = sum, geom = "bar", position = "dodge") +
      theme_minimal() +
      labs(title = "Peptides per sample", x = "Condition", y = "Number of peptides", fill = "Condition") +
      theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
RTvspredRT_plot <- function(data2, meta, method="Hexbin Plot", add_line=FALSE, bins=1000){
  ...
  if (method=="Scatter Plot"){
    ...
    plot <- ggplot(data2, aes(x = Predicted.RT, y = RT)) +
      ...
      theme_minimal() +
      labs(title = meta$title, x = "Predicted RT", y = "RT")
  } else if (method=="Density Plot"){
    plot <- ggplot(data2, aes(x = Predicted.RT, y = RT)) +
      stat_density_2d() +
      labs(x = "Predicted RT", y = "Actual RT", title = "Predicted vs Actual Retention Time") +
      theme_minimal()
...
modification_plot <- function(data2, meta, id=TRUE){
  ...
  data_carb <- data_filtered %>%
    filter(str_detect(Modified.Sequence, "UniMod:4") | 
             str_detect(Modified.Sequence, "Carbamidomethyl"))
  data_carb <- data_carb %>%
    mutate(across(-Modified.Sequence, ~ ifelse(is.na(.), 0, 1)))
  col_sums_carb <- colSums(data_carb[, -1])
  
  data_oxi <- data_filtered %>%
    filter(str_detect(Modified.Sequence, "UniMod:35") | 
             str_detect(Modified.Sequence, "Oxidation"))
  data_oxi <- data_oxi %>%
    mutate(across(-Modified.Sequence, ~ ifelse(is.na(.), 0, 1)))
  col_sums_oxi <- colSums(data_oxi[, -1])
  
  data_ace <- data_filtered %>%
    filter(str_detect(Modified.Sequence, "UniMod:1") | 
             str_detect(Modified.Sequence, "Acetyl"))
  data_ace <- data_ace %>%
    mutate(across(-Modified.Sequence, ~ ifelse(is.na(.), 0, 1)))
  col_sums_ace <- colSums(data_ace[, -1])
  ...
  ggplot(plot_data, aes(x = Sample, y = Count, fill = Modification)) +
    ...
    labs(title = "Modifications per sample", x = "Sample", y = "Number of modified peptides") +
...
missed_cl_plot <- function(data2, meta, id = TRUE) {
  ...
  ggplot(plot_data, aes(x = Sample, y = Percentage, fill = factor(Count))) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    geom_text(data = missed_cleavage_labels, 
...
prepare_KSEA <- function(data, meta, condition1, condition2) {
  annotated_columns1 <- meta$sample[meta$condition == condition1]
  annotated_columns2 <- meta$sample[meta$condition == condition2]
  data_filtered <- data[, c("PTM_Collapse_key", "UPD_seq", annotated_columns1, annotated_columns2), drop = FALSE]
  ...
  data_filtered$FC = log2fc
  data <- data_filtered %>%
    select(UPD_seq, FC) %>%
    filter(!is.na(FC))
  
  colnames(data)[colnames(data) == "UPD_seq"] <- "site sequence"
  colnames(data)[colnames(data) == "FC"] <- "log fold change"
...
init_colors <- function(color){
  if (color == "Default") {
    ...
    plot_colors <<- c( "#F8766D", "#E68613", "#CD9600", "#ABA300", "#7CAE00", "#0BB702","#00BE67", "#00C19A", "#00BFC4", "#00B8E7", "#00A9FF", "#8494FF","#C77CFF", "#ED68ED", "#FF61CC", "#FF68A1","black")
  } else if (color == "Warm/Cold"){
    plot_colors <<- c("blue","red","green","orange","purple","darkgoldenrod","cyan","gold","orchid","salmon","navy","brown","steelblue","coral","skyblue","peachpuff","orchid")
  } else if (color == "Black/Grey"){
    plot_colors <<- c("black", "grey")
  } else if (color == "Yue7"){
    plot_colors <<- c("#2D5F85","#5184B2","#AAD4F8","#F2F5FA","#F1A7B5","#D55276","#AB3A54")
  }
}
...
dim_func <- function(data, meta, method, color_package = TRUE){
  plot_obj <- switch(method,
                     ...
                     "tSNE"      = tsne_plot(data, meta, color_package),
                     "UMAP"      = umap_plot(data, meta, color_package)
  )
  print(plot_obj)
}
